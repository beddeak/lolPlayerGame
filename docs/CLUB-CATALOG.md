...
선수 능력치는 사용자가 입력한 값을 유지한다. 현재 HLE, GEN, DK, BLG, AL, G2, FNC,
LYON, FLY의 9개 구단에 45명이 배정되어 있다. 미배정 T1 선수 5명까지 선수 카드는 총 50명이다.
개발용 가상 선수 14명,
...
- 이번 정리는 파일 기준이다. 기존 DB에 이미 들어간 테스트 선수·세트효과나 옛 세이브를
  삭제하지 않았다. JSON에서 빠진 행을 자동 삭제하지 않으므로 별도 DB 정리가 필요하면
  기존 저장의 참조를 먼저 확인해야 한다.
