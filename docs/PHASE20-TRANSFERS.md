...
- `CONTRACTED`: 다른 커리어 구단에 소속되고 활성 계약이 있는 선수

감독 소속 구단의 선수는 영입 시장 후보에서 제외한다. 계약이 없는 상태인데 팀이나 로스터만
남아 있는 것처럼 데이터가 서로 모순되는 선수는 시장에 노출하지 않고, 쓰기 요청에서도 거절한다.
...
선수의 소속 또는 활성 계약이 바뀌었거나 이미 다른 영입에 사용된 합의는 재사용할 수 없다.
...
- 같은 선수에 대한 더 이상 유효하지 않은 열린 제안과 이벤트 정리
